...
def main():
    """Main function to process static WAV file or real-time microphone input."""
    print("Welcome to Speech Transcription")
    print("Choose input mode:")
    print("1. Static audio file (WAV only)")
    print("2. Real-time microphone input (controlled by space bar)")
    ...
    if mode == "1":
        print(f"Place audio files in '{INPUT_AUDIO_DIR}' directory.")
        audio_file = input("Enter filename (e.g., sample.wav) or full path: ")
        
        # Validate input file
        audio_path = validate_file_path(audio_file)
        if not audio_path:
        ...
        text, language = transcribe_audio_file(audio_path)
    elif mode == "2":
        # Transcribe microphone input with space bar control
        print("Starting real-time transcription...")
        text, language = transcribe_microphone()
    else:
    ...
    # Save transcription
    if text:
        save_transcription(text, language, output_file)
    else:
